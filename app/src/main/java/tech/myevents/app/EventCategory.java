...
public class EventCategory extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (from.equals(""))
            changeBusinessSize(prefs.getString(AppInfo.EVENT_INTEREST, ""));
        else
            changeBusinessSize(prefs.getString(AppInfo.EVENT_INTEREST_EX, ""));

        String pageName = "Event Category";
    ...
    private void changeBusinessSize(String interest) {
        editor = prefs.edit();
        if (from.equals(""))
            editor.putString(AppInfo.EVENT_INTEREST, interest);
        else
            editor.putString(AppInfo.EVENT_INTEREST_EX, interest);
        ...
        for (int x = 0; x < StaticVariables.categories.length; x++) {
            if (StaticVariables.categories[x].equals(interest)) {
                ...
                if (from.equals(""))
                    editor.putString(AppInfo.EVENT_INTEREST_CODE, StaticVariables.interestCodes[x]);
                else
                    editor.putString(AppInfo.EVENT_INTEREST_CODE_EX, StaticVariables.interestCodes[x]);
            } else
