...
import static tech.myevents.app.BroadcastAd.promotionCode;

public class PaymentsAd extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ad_payments);
        ...
        tvPromoCode.setText("the promotion code is: " + setterPromo.getAdPaymentCode() + "");
    ...
    private void confirmPromotionCode() {
        if (BroadcastAd.promotionCode == null) {
            Toast.makeText(getApplicationContext(), "Network Error.", Toast.LENGTH_LONG).show();
            finish();
        }
        progressBar.setVisibility(View.VISIBLE);
        String promoCode = setterPromo.getAdPaymentCode();
        ...
        if (!result) {
            editor = prefs.edit();
            editor.putBoolean(AppInfo.AD_BROADCAST_PAYMENT, true);
            editor.putString(AppInfo.AD_APPROVAL_CODE, String.valueOf(promoCode));
            editor.apply();
            addApprovalCodeToDB(StaticVariables.free, promoCode, "ad payment",setterPromo.getAdValidUntil());

            setterA = new GetSetterApprovalCode(StaticVariables.free, promoCode, "ad payment", setterPromo.getAdValidUntil());
            ...
            Toast.makeText(getApplicationContext(), "Code was valid: You can now broadcast.", Toast.LENGTH_LONG).show();
            finish();
        } else {
            progressBar.setVisibility(View.INVISIBLE);
    ...
    private void payUsingEcocash() {
        ...
        if (prefs.getString(AppInfo.AD_TITLE, "").isEmpty()
                || prefs.getString(AppInfo.AD_BRAND_NAME, "").isEmpty()
                || prefs.getString(AppInfo.AD_LOCATION, "").isEmpty()
                || prefs.getString(AppInfo.AD_INTEREST_CODE, "").equals("")
                || prefs.getLong(AppInfo.AD_DURATION_CODE, 0) == 0) {
            Toast.makeText(getApplicationContext(), "Fill out all details in your ad form", Toast.LENGTH_LONG).show();
        } else {
            progressBar.setVisibility(View.VISIBLE);
    ...
    private void requestUSSD() {
        ...
        String cost = String.valueOf(prefs.getInt(AppInfo.AD_COST, 2));
        String sendMoney = "*151*1*1*"+ ecocashNumber + "*" + cost;
        String harsh = Uri.encode("#");

        if (ActivityCompat.checkSelfPermission(PaymentsAd.this, android.Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {
            return;
        }
        startSmsListen();
                ...
                if (c.getPosition() < 8) {
                    ...
                    if (number.contains("+263164") && body.toLowerCase().contains(name.toLowerCase()) && body.contains(cost + ".00") && body.contains("Transfer Confirmation")) {
                        ...
                        if (!result) {
                            ...
                            editor = prefs.edit();
                            editor.putBoolean(AppInfo.AD_BROADCAST_PAYMENT, true);
                            editor.putString(AppInfo.AD_APPROVAL_CODE, approvalCode);
                            editor.apply();
                            paymentHandler.removeCallbacks(runnablePaymentData);

                            addApprovalCodeToDB("", approvalCode, "ad payment", getCurrentTimestamp());

                            setterA = new GetSetterApprovalCode("", approvalCode, "ad payment", getCurrentTimestamp());
